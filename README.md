...
// $db = \Devinow\Db\PdoDatabase::fromDsn(new \Devinow\Db\PdoDsn('mysql:dbname=my-database;host=localhost;charset=utf8mb4', 'my-username', 'my-password'));
// or
// $db = \Devinow\Db\PdoDatabase::fromDsn(new \Devinow\Db\PdoDsn('pgsql:dbname=my-database;host=localhost;port=5432', 'my-username', 'my-password'));
// or
// $db = \Devinow\Db\PdoDatabase::fromDsn(new \Devinow\Db\PdoDsn('sqlite:../Databases/my-database.sqlite'));

$auth = new \Devinow\Auth\Auth($db);
```
...
catch (\Devinow\Auth\InvalidEmailException $e) {
    die('Invalid email address');
}
catch (\Devinow\Auth\InvalidPasswordException $e) {
    die('Invalid password');
}
catch (\Devinow\Auth\UserAlreadyExistsException $e) {
    die('User already exists');
}
catch (\Devinow\Auth\TooManyRequestsException $e) {
...
catch (\Devinow\Auth\EmailNotVerifiedException $e) {
...
catch (\Devinow\Auth\InvalidSelectorTokenPairException $e) {
    die('Invalid token');
}
catch (\Devinow\Auth\TokenExpiredException $e) {
    die('Token expired');
}
catch (\Devinow\Auth\UserAlreadyExistsException $e) {
...
catch (\Devinow\Auth\ResetDisabledException $e) {
...
catch (\Devinow\Auth\NotLoggedInException $e) {
    die('Not logged in');
}
catch (\Devinow\Auth\InvalidPasswordException $e) {
...
catch (\Devinow\Auth\ConfirmationRequestNotFound $e) {
...
Here’s how to use this library with your own tables for custom user information in a maintainable and re-usable way:
 ...
 1. If you need the custom user information only rarely, you may just retrieve it as needed. If you need it more frequently, however, you’d probably want to have it in your session data. The following method is how you can load and access your data in a reliable way:
    ...
    ```php
    function getUserInfo(\Devinow\Auth\Auth $auth) {
        if (!$auth->isLoggedIn()) {
            return null;
        }
...
```php
if ($auth->hasRole(\Devinow\Auth\Role::SUPER_MODERATOR)) {
    echo 'The user is a super moderator';
}

// or

if ($auth->hasAnyRole(\Devinow\Auth\Role::DEVELOPER, \Devinow\Auth\Role::MANAGER)) {
    echo 'The user is either a developer, or a manager, or both';
}

// or

if ($auth->hasAllRoles(\Devinow\Auth\Role::DEVELOPER, \Devinow\Auth\Role::MANAGER)) {
...
```php
\Devinow\Auth\Role::ADMIN;
\Devinow\Auth\Role::AUTHOR;
\Devinow\Auth\Role::COLLABORATOR;
\Devinow\Auth\Role::CONSULTANT;
\Devinow\Auth\Role::CONSUMER;
\Devinow\Auth\Role::CONTRIBUTOR;
\Devinow\Auth\Role::COORDINATOR;
\Devinow\Auth\Role::CREATOR;
\Devinow\Auth\Role::DEVELOPER;
\Devinow\Auth\Role::DIRECTOR;
\Devinow\Auth\Role::EDITOR;
\Devinow\Auth\Role::EMPLOYEE;
\Devinow\Auth\Role::MAINTAINER;
\Devinow\Auth\Role::MANAGER;
\Devinow\Auth\Role::MODERATOR;
\Devinow\Auth\Role::PUBLISHER;
\Devinow\Auth\Role::REVIEWER;
\Devinow\Auth\Role::SUBSCRIBER;
\Devinow\Auth\Role::SUPER_ADMIN;
\Devinow\Auth\Role::SUPER_EDITOR;
\Devinow\Auth\Role::SUPER_MODERATOR;
\Devinow\Auth\Role::TRANSLATOR;
```
...
```php
\Devinow\Auth\Role::getMap();
// or
\Devinow\Auth\Role::getNames();
// or
\Devinow\Auth\Role::getValues();
```
...
```php
function canEditArticle(\Devinow\Auth\Auth $auth) {
...
final class MyRole {
    ...
    const CUSTOMER_SERVICE_AGENT = \Devinow\Auth\Role::REVIEWER;
    const FINANCIAL_DIRECTOR = \Devinow\Auth\Role::COORDINATOR;
...
catch (\Devinow\Auth\UnknownIdException $e) {
    die('Unknown ID');
}
```
...
catch (\Devinow\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
catch (\Devinow\Auth\AmbiguousUsernameException $e) {
...
```php
try {
    $auth->admin()->addRoleForUserById($userId, \Devinow\Auth\Role::ADMIN);
}
catch (\Devinow\Auth\UnknownIdException $e) {
    die('Unknown user ID');
}
...
try {
    $auth->admin()->addRoleForUserByEmail($userEmail, \Devinow\Auth\Role::ADMIN);
}
catch (\Devinow\Auth\InvalidEmailException $e) {
    die('Unknown email address');
}
...
try {
    $auth->admin()->addRoleForUserByUsername($username, \Devinow\Auth\Role::ADMIN);
}
catch (\Devinow\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
...
```php
try {
    $auth->admin()->removeRoleForUserById($userId, \Devinow\Auth\Role::ADMIN);
}
catch (\Devinow\Auth\UnknownIdException $e) {
    die('Unknown user ID');
}
...
try {
    $auth->admin()->removeRoleForUserByEmail($userEmail, \Devinow\Auth\Role::ADMIN);
}
catch (\Devinow\Auth\InvalidEmailException $e) {
    die('Unknown email address');
}
...
try {
    $auth->admin()->removeRoleForUserByUsername($username, \Devinow\Auth\Role::ADMIN);
}
catch (\Devinow\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
...
```php
try {
    if ($auth->admin()->doesUserHaveRole($userId, \Devinow\Auth\Role::ADMIN)) {
...
```php
\Devinow\Auth\Auth::createRememberCookieName();
```
...
```php
$length = 24;
$randomStr = \Devinow\Auth\Auth::createRandomString($length);
```

#### Creating a UUID v4 as per RFC 4122

```php
$uuid = \Devinow\Auth\Auth::createUuid();
```

### Reading and writing session data

For detailed information on how to read and write session data conveniently, please refer to [the documentation of the session library](https://github.com/devinow/cookie#reading-and-writing-session-data), which is included by default.
